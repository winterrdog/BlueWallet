...
def cmp_resources_arsc(first_entry_bytes: bytes, second_entry_bytes: bytes) -> bool:
    ...
    for package_name in all_package_names:
        ...
        for idx, (pkg1, pkg2) in enumerate(
            tqdm(zip(packages1, packages2), total=len(packages1))
        ):
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        logger.error("Not enough arguments. Usage: apkdiff <path_to_first_apk> <path_to_second_apk>")
        sys.exit(1)
